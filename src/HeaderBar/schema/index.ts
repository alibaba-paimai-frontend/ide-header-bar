 ...
 * HeaderBar 对应的模型
 */
export const HeaderBarModel = types
  ...
  .actions(self => {
    return {
      ...
      setTheme(theme: IHeaderBarTheme) {
        let target = theme;
        if (typeof theme === 'string') {
          target = JSON.parse(theme);
        }
        self._theme = cast(target);
      },
      // 生成一个新的 style
  ...
  .actions(self => {
    return {
      ...
      updateAttribute(name: string, value: any) {
        return updateModelAttribute(self as IHeaderBarModel, name, value);
      },
